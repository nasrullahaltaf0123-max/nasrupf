        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 md:gap-6">
          {testimonials.map((t, i) => (
            ...
            >
              ...
              <Quote className="w-5 h-5 text-neon-purple/30 mb-3" />
              <p className="text-sm text-foreground/85 leading-relaxed mb-5 italic">"{t.text}"</p>

              <div className="flex items-center gap-1 mb-3">
                {[...Array(5)].map((_, j) => (
                  <Star key={j} className="w-3.5 h-3.5 fill-neon-cyan text-neon-cyan" style={{ filter: 'drop-shadow(0 0 3px hsl(var(--neon-cyan) / 0.4))' }} />
                ))}
              </div>
