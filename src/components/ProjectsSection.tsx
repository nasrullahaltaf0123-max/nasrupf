...
const ProjectCard = memo(({ p, i, onClick }: { p: DigitalLabProduct; i: number; onClick: () => void }) => {
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className={`group block rounded-2xl relative overflow-hidden cursor-pointer transition-all duration-300 ease-out hover:scale-[1.02] hover:-translate-y-1 ${isMobile && isPressed ? 'mobile-tap-glow' : ''}`}
      style={{
    ...
    >
      ...
      <div className="flex items-start gap-4">
        <div
          className="w-12 h-12 md:w-14 md:h-14 rounded-xl flex items-center justify-center flex-shrink-0 transition-shadow duration-300 group-hover:shadow-[0_0_14px_hsl(var(--neon-cyan)/0.2)]"
          style={{
            background: `hsl(var(${p.accent}) / 0.06)`,
            border: `1px solid hsl(var(${p.accent}) / 0.08)`,
          }}
        >
          <Icon className="w-6 h-6 md:w-7 md:h-7 text-neon-cyan group-hover:scale-110 transition-transform duration-300" style={{ filter: 'drop-shadow(0 0 3px hsl(var(--neon-cyan) / 0.3))' }} />
        </div>
      ...
      <AnimatePresence>
        {selectedProject !== null && (() => {
          ...
          return (
            ...
            >
              <div className="absolute inset-0 bg-background/80" style={{ backdropFilter: 'blur(10px)' }} />
              <motion.div
                ...
                style={{
                  ...
                  boxShadow: `0 0 40px hsl(var(${project.accent}) / 0.08), 0 8px 32px hsl(0 0% 0% / 0.35)`,
                }}
              >
                <button
                  onClick={() => setSelectedProject(null)}
                ...
                <div className="text-center">
                  ...
                  >
                    <Icon className="w-7 h-7 text-neon-cyan" style={{ filter: 'drop-shadow(0 0 4px hsl(var(--neon-cyan) / 0.4))' }} />
                  </div>
                  ...
                  <div className="flex gap-3 justify-center">
                    <a
                      ...
                      style={{
                        ...
                        boxShadow: '0 0 20px hsl(var(--neon-purple) / 0.15), 0 4px 12px hsl(0 0% 0% / 0.25)',
                      }}
                    >
                      Live Project
